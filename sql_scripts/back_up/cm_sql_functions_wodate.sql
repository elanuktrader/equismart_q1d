DROP FUNCTION IF EXISTS compute_cm_features(integer, character varying);
CREATE OR REPLACE FUNCTION compute_cm_features(mov_avg_span INTEGER, stock_symbol CHARACTER VARYING(50) DEFAULT NULL)
RETURNS TABLE (
    ...
    WITH 
        ...
        day_closure AS (
            SELECT
                ...
                (((sr.high - sr.low)/(sr.high + sr.low))*100)<0.1 AS small_rng
...
DROP FUNCTION IF EXISTS compute_cm_summary(mov_avg_span integer, stock_symbol character varying);
CREATE OR REPLACE FUNCTION compute_cm_summary(mov_avg_span integer, stock_symbol character varying(50) default NULL)
RETURNS TABLE (
    nse_symbol character varying(50),
    date date,
    spk_t_vr SMALLINT,
...
BEGIN
    ...
    WITH feature_data AS (
        SELECT * FROM compute_cm_features(mov_avg_span, stock_symbol)
    )
    SELECT 
