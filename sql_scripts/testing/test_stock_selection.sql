DROP FUNCTION IF EXISTS test_stock_selection_summary(mov_avg_span integer, stock_symbols TEXT[],fetch_date date, insert_date date);
CREATE OR REPLACE FUNCTION test_stock_selection_summary(
...
BEGIN
    ...
    WITH feature_data AS (
        SELECT * 
        FROM test_stock_selection_features(mov_avg_span, stock_symbols,fetch_date)
...
DROP FUNCTION IF EXISTS test_stock_selection_features(integer, TEXT[],date);
CREATE OR REPLACE FUNCTION test_stock_selection_features(
    mov_avg_span INTEGER, 
    stock_symbols TEXT[] DEFAULT NULL,
    fetch_date DATE DEFAULT NULL)
RETURNS TABLE (
...
BEGIN
    ...
    WITH 
        ...
        daily_volume AS (
            ...
            WHERE ((stock_symbols IS NULL OR nse.nse_symbol = ANY(stock_symbols)) AND (nse."timestamp"::date>=fetch_date or fetch_date is NULL )  )
